 ...
 * ================================================================ */
typedef struct {
    /* Main capture pipeline (always running, resilient) */
    GstElement  *pipeline;
    GstElement  *tee;
    GMainLoop   *loop;
    ...
    /* Recording Branch (ISOLATED PIPELINE via appsink -> appsrc bridge) */
    GstElement  *rec_pipeline;   /* Completely separate GStreamer pipeline! */
    GstElement  *rec_main_q;     /* Queue in the main pipeline */
    GstElement  *rec_main_sink;  /* Appsink in the main pipeline */
    GstPad      *rec_teepad;
    
    GstElement  *rec_appsrc;     /* Appsrc in the isolated pipeline */
    guint64      rec_base_pts;
    gboolean     rec_base_set;
...
/* ================================================================
 * Build resilient main capture pipeline (src -> tee)
 * ================================================================ */
static void build_capture_pipeline(const char *device) {
    ...
    /* NV12 flows directly from camera to tee — no G2D format conversion needed */
    ...
    gst_bin_add_many(GST_BIN(ctx.pipeline), src, src_caps, ctx.tee, dummy_q, dummy_sink, ctx.disp_q, ctx.disp_sink, NULL);
    gst_element_link_many(src, src_caps, ctx.tee, NULL);
...
static void add_rtsp_branch(void) {
    ...
    g_object_set(ctx.rtsp_q, "max-size-time", (guint64)1000000000, "leaky", 2, NULL);
    const char *g2d = gst_element_factory_find("imxvideoconvert_g2d") ? "imxvideoconvert_g2d" : "videoconvert";
    ctx.rtsp_scale = gst_element_factory_make(g2d, "rtsp_scale");
    ctx.rtsp_res_caps = gst_element_factory_make("capsfilter", "rtsp_caps");
    char c[128]; snprintf(c, sizeof(c), "video/x-raw,width=%d,height=%d", SRC_W, SRC_H);
    GstCaps *rc = gst_caps_from_string(c); g_object_set(ctx.rtsp_res_caps, "caps", rc, NULL); gst_caps_unref(rc);
    ...
    gst_bin_add_many(GST_BIN(ctx.pipeline), ctx.rtsp_q, ctx.rtsp_scale, ctx.rtsp_res_caps, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink, NULL);
    gst_element_link_many(ctx.rtsp_q, ctx.rtsp_scale, ctx.rtsp_res_caps, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink, NULL);
    
    GstElement *ee[] = {ctx.rtsp_q, ctx.rtsp_scale, ctx.rtsp_res_caps, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink};
    for(int i=0; i<7; i++) gst_element_sync_state_with_parent(ee[i]);
...
/* ================================================================
 * RECORDING BRANCH (DIRECT TEE ARCHITECTURE)
 * ================================================================ */
...
static void add_rec_branch(const char *p) {
    ...
    g_print("Adding DIRECT Recording Pipeline: %dx%d @ %dfps | codec=%s | bitrate=%d\n", w, h, fps, codec, br);

    rec_n_elems = 0;

    GstElement *q = gst_element_factory_make("queue", "rq");
    g_object_set(q, "max-size-time", (guint64)1000000000ULL, "leaky", 2, NULL);
    ...
    char c2[256]; snprintf(c2, sizeof(c2), "video/x-raw,width=%d,height=%d", w, h);
    ...
    g_print("  -> Recording pipeline linked DIRECTLY to TEE.\n");
}
...
static gboolean dot_dump_cb(gpointer d) {
    ...
    tick += 10;
    char name[128];
    snprintf(name, sizeof(name), "test_pipeline_tick_%03d", tick);
