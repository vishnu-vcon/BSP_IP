...
static gboolean dot_dump_cb(gpointer d) {
    ...
    tick += 10;
    char name[128];
    snprintf(name, sizeof(name), "test_rgb16_global_tick_%03d", tick);
...
static void build_capture_pipeline(const char *device) {
    ...
    char c2[256]; snprintf(c2, sizeof(c2), "video/x-raw,format=RGB16,width=%d,height=%d", SRC_W, SRC_H);
...
static void add_rec_branch(const char *p) {
    ...
    g_print("Adding Recording: RGB16->G2D->RGB16->Queue->Encoder %dx%d @ %dfps | codec=%s\n", w, h, fps, codec);
    ...
    char c2[256]; snprintf(c2, sizeof(c2), "video/x-raw,format=RGB16,width=%d,height=%d", w, h);
