...
typedef struct {
    GstElement  *pipeline;
    GstElement  *nv12_tee;  /* First Tee */
    GstElement  *bgrx_tee;  /* Second Tee */
    GMainLoop   *loop;
...
static gboolean dot_dump_cb(gpointer d) {
    ...
    tick += 10;
    char name[128];
    snprintf(name, sizeof(name), "test_dual_tee_tick_%03d", tick);
...
/* ================================================================
 * Build resilient main capture pipeline (src -> NV12_TEE -> BGRX_TEE)
 * ================================================================ */
static void build_capture_pipeline(const char *device) {
    ...
    ctx.nv12_tee = gst_element_factory_make("tee", "nv12_tee");
    g_object_set(ctx.nv12_tee, "allow-not-linked", TRUE, NULL);

    GstElement *dq1 = gst_element_factory_make("queue", "dq1");
    g_object_set(dq1, "max-size-buffers", 2, "leaky", 2, NULL);
    GstElement *ds1 = gst_element_factory_make("fakesink", "ds1");
    g_object_set(ds1, "sync", FALSE, NULL);

    /* The GLOBAL G2D that creates the second BGRx TEE */
    GstElement *q_g2d = gst_element_factory_make("queue", "q_g2d");
    ...
    GstElement *global_g2d = gst_element_factory_make(g2n, "global_g2d");
    
    GstElement *global_caps = gst_element_factory_make("capsfilter", "global_caps");
    char c2[256]; snprintf(c2, sizeof(c2), "video/x-raw,format=BGRx,width=%d,height=%d,colorimetry=1:1:16:4", SRC_W, SRC_H);
    GstCaps *sc2 = gst_caps_from_string(c2); g_object_set(global_caps, "caps", sc2, NULL); gst_caps_unref(sc2);
    
    ctx.bgrx_tee = gst_element_factory_make("tee", "bgrx_tee");
    g_object_set(ctx.bgrx_tee, "allow-not-linked", TRUE, NULL);

    GstElement *dq2 = gst_element_factory_make("queue", "dq2");
    g_object_set(dq2, "max-size-buffers", 2, "leaky", 2, NULL);
    GstElement *ds2 = gst_element_factory_make("fakesink", "ds2");
    g_object_set(ds2, "sync", FALSE, NULL);
    ...
    gst_bin_add_many(GST_BIN(ctx.pipeline), src, src_caps, ctx.nv12_tee, dq1, ds1, 
                     q_g2d, global_g2d, global_caps, ctx.bgrx_tee, dq2, ds2, ctx.disp_q, ctx.disp_sink, NULL);
                     
    /* SOURCE -> NV12 TEE */
    gst_element_link_many(src, src_caps, ctx.nv12_tee, NULL);

    GstPad *tp1 = gst_element_request_pad_simple(ctx.nv12_tee, "src_%u");
    GstPad *qp1 = gst_element_get_static_pad(dq1, "sink");
    gst_pad_link(tp1, qp1); gst_object_unref(qp1); gst_object_unref(tp1);
    gst_element_link(dq1, ds1);

    /* NV12 TEE -> GLOBAL G2D -> BGRX TEE */
    GstPad *tp_g2d = gst_element_request_pad_simple(ctx.nv12_tee, "src_%u");
    GstPad *qp_g2d = gst_element_get_static_pad(q_g2d, "sink");
    gst_pad_link(tp_g2d, qp_g2d); gst_object_unref(qp_g2d); gst_object_unref(tp_g2d);
    gst_element_link_many(q_g2d, global_g2d, global_caps, ctx.bgrx_tee, NULL);

    /* BGRX TEE -> Dummy 2 */
    GstPad *tp2 = gst_element_request_pad_simple(ctx.bgrx_tee, "src_%u");
    GstPad *qp2 = gst_element_get_static_pad(dq2, "sink");
    gst_pad_link(tp2, qp2); gst_object_unref(qp2); gst_object_unref(tp2);
    gst_element_link(dq2, ds2);

    /* BGRX TEE -> DISP SINK */
    GstPad *tp_disp = gst_element_request_pad_simple(ctx.bgrx_tee, "src_%u");
    GstPad *qp_disp = gst_element_get_static_pad(ctx.disp_q, "sink");
    gst_pad_link(tp_disp, qp_disp); gst_object_unref(qp_disp); gst_object_unref(tp_disp);
...
/* ================================================================
 * RTSP BRIDGING (Connects to BGRx TEE)
 * ================================================================ */
...
static void add_rtsp_branch(void) {
    ...
    int fps = 25;
    /* Takes pre-scaled BGRx from BGRx TEE */
    ...
    ctx.rtsp_teepad = gst_element_request_pad_simple(ctx.bgrx_tee, "src_%u");
    GstPad *sp = gst_element_get_static_pad(ctx.rtsp_q, "sink");
    gst_pad_link(ctx.rtsp_teepad, sp); gst_object_unref(sp);
    
    g_print("Added RTSP branch. (Fed by BGRX TEE)\n");
    ctx.rtsp_attached = TRUE;
}

/* ================================================================
 * RECORDING BRANCH (Connects to NV12 TEE)
 * ================================================================ */
...
static void add_rec_branch(const char *p) {
    if (ctx.rec_attached) {
        g_print("Recording branch already attached. Stop first.\n");
        return;
    }
    ...
    g_print("Adding Recording Branch (Fed by NV12 Tee -> Individual G2D): %dx%d @ %dfps | codec=%s | bitrate=%d\n", w, h, fps, codec, br);
    ...
    const char *en = (g_strcmp0(codec, "h265") == 0) ? "v4l2h265enc" : (g_strcmp0(codec, "x264") == 0 ? "x264enc" : "v4l2h264enc");
    ...
    for (int i=0; i < rec_n_elems; i++) {
        gst_bin_add(GST_BIN(ctx.pipeline), rec_elems[i]);
    }
    
    for (int i=0; i < rec_n_elems - 1; i++) {
        gst_element_link(rec_elems[i], rec_elems[i+1]);
    }
    
    for (int i=0; i < rec_n_elems; i++) {
        gst_element_sync_state_with_parent(rec_elems[i]);
    }
    
    ctx.rec_frame_count = 0;
    
    ctx.rec_teepad = gst_element_request_pad_simple(ctx.nv12_tee, "src_%u");
    GstPad *sp = gst_element_get_static_pad(scale, "sink");
    if(gst_pad_link(ctx.rec_teepad, sp) != GST_PAD_LINK_OK) {
        g_print("Failed to link rec branch\n");
    }
    gst_object_unref(sp);
...
static GstPadProbeReturn rec_unlink_probe(GstPad *p, GstPadProbeInfo *i, gpointer d) {
    ...
    gst_pad_unlink(p, sp); 
    gst_element_release_request_pad(ctx.nv12_tee, p);
