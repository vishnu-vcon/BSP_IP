...
static void build_capture_pipeline(const char *device) {
    ...
    /* GLOBAL G2D: Convert NV12 to BGRx once before the Tee (Iteration 3 Arch) */
    ...
    GstElement *disp_g2d = gst_element_factory_make(g2n, "disp_g2d");
    ...
    gst_bin_add_many(GST_BIN(ctx.pipeline), src, src_caps, global_g2d, global_caps, ctx.tee, dummy_q, dummy_sink, ctx.disp_q, disp_g2d, ctx.disp_sink, NULL);
    gst_element_link_many(src, src_caps, global_g2d, global_caps, ctx.tee, NULL);
    ...
    gst_element_link_many(ctx.disp_q, disp_g2d, ctx.disp_sink, NULL);
...
static void add_rtsp_branch(void) {
    ...
    gst_bin_add_many(GST_BIN(ctx.pipeline), ctx.rtsp_q, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink, NULL);
    gst_element_link_many(ctx.rtsp_q, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink, NULL);
    
    GstElement *ee[] = {ctx.rtsp_q, ctx.rtsp_rate, ctx.rtsp_enc, ctx.rtsp_parse, ctx.rtsp_sink};
    for(int i=0; i<5; i++) gst_element_sync_state_with_parent(ee[i]);
...
static void add_rec_branch(const char *p) {
    ...
    g_print("Adding DIRECT Recording Pipeline (Iteration 3 - Resize Only): %dx%d @ %dfps | codec=%s | bitrate=%d\n", w, h, fps, codec, br);
    ...
    g_print("  -> BGRx Recording pipeline linked DIRECTLY to TEE.\n");
}
...
static gboolean dot_dump_cb(gpointer d) {
    ...
    tick += 10;
    char name[128];
    snprintf(name, sizeof(name), "test_bgrx_pipeline_tick_%03d", tick);
