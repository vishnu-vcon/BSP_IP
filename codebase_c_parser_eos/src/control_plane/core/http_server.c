...
static void _make_session(const char *user, const char *role, Session *out)
{
    ...
    g_strlcpy(out->user, user, USERNAME_MAX);
    g_strlcpy(out->role, role, ROLE_MAX);
    out->active     = true;
...
static void _user_list_cb(const char *username, const char *role,
                           const char *email, void *user_data)
{
    JsonBuilder *b = (JsonBuilder *)user_data;
    ...
    json_builder_add_string_value(b, email);
                           ...
                           GHashTable *query, SoupClientContext *client, gpointer user_data)
{
    ...
    if (g_strcmp0(msg->method, "GET") == 0) {
        ...
        db_manager_list_users(_user_list_cb, b);
                                ...
                                GHashTable *query, SoupClientContext *client, gpointer user_data)
{
    ...
    if (g_strcmp0(filename, "playlist.m3u8") == 0 || g_str_has_suffix(filename, ".ts")) {
...
static void _handle_system_smtp(SoupServer *server, SoupMessage *msg, const char *path,
                                GHashTable *query, SoupClientContext *client, gpointer user_data)
{
    (void)server; (void)path; (void)client;
    HTTPServer *srv = (HTTPServer *)user_data;
    if (!http_server_authorize(srv, msg, query, "admin")) return;

    if (g_strcmp0(msg->method, "GET") == 0) {
        SmtpServerConfig cfg;
        if (get_smtp_config(&cfg)) {
            char *json = g_strdup_printf(
                "{\"host\":\"%s\",\"port\":%d,\"user\":\"%s\",\"pass\":\"***\",\"from\":\"%s\",\"use_ssl\":%s}",
                cfg.host, cfg.port, cfg.username, cfg.mail_from, cfg.use_ssl ? "true" : "false");
            http_server_respond_json(msg, 200, json);
            g_free(json);
        } else {
            http_server_respond_error(msg, 500, "Failed to load SMTP config");
        }
    } else if (g_strcmp0(msg->method, "PUT") == 0) {
        SoupBuffer *body; JsonParser *parser;
        JsonObject *obj = _parse_body(msg, &body, &parser);
        if (!obj) return;
        ...
        if (json_object_has_member(obj, "host")) g_strlcpy(cfg.host, json_object_get_string_member(obj, "host"), sizeof(cfg.host));
        if (json_object_has_member(obj, "port")) cfg.port = json_object_get_int_member(obj, "port");
        if (json_object_has_member(obj, "user")) g_strlcpy(cfg.username, json_object_get_string_member(obj, "user"), sizeof(cfg.username));
        if (json_object_has_member(obj, "pass")) g_strlcpy(cfg.password, json_object_get_string_member(obj, "pass"), sizeof(cfg.password));
        if (json_object_has_member(obj, "from")) g_strlcpy(cfg.mail_from, json_object_get_string_member(obj, "from"), sizeof(cfg.mail_from));
        if (json_object_has_member(obj, "use_ssl")) cfg.use_ssl = json_object_get_boolean_member(obj, "use_ssl");
...
HTTPServer *http_server_new(TokenAuth *auth, int port, gboolean enable_tls)
{
    ...
    soup_server_add_handler(srv->soup, "/api/v1/system/smtp", _handle_system_smtp, srv, NULL);
