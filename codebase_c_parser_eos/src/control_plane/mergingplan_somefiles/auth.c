...
#define DEFAULT_SMTP_CONF  "/etc/smartip/smtp.conf"
                          ...
                          const char *smtp_conf_path)
{
    ...
    /* Load SMTP config (non-fatal) */
    const char *conf = smtp_conf_path ? smtp_conf_path : DEFAULT_SMTP_CONF;
    SmtpConfig full_cfg;
    memset(&full_cfg, 0, sizeof(full_cfg));
    if (load_config(conf, &full_cfg) == 0) {
        auth->smtp        = full_cfg.server;
        auth->smtp_loaded = TRUE;
        if (full_cfg.recipient_count > 0)
            g_strlcpy(auth->smtp_recipient, full_cfg.recipients[0],
                      sizeof(auth->smtp_recipient));
        g_info("[Auth] SMTP loaded from %s", conf);
    } else {
        g_warning("[Auth] SMTP config not found at %s — OTP email disabled", conf);
    }
