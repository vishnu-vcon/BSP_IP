...
static void _on_media_configure(GstRTSPMediaFactory *factory,
    ...
    g_info("[RTSP] Client connected to '%s'  codec=%s  %dx%d @ %d fps",
           rtsp->mount_path, rtsp->codec, rtsp->w, rtsp->h, rtsp->fps);
    ...
    BranchConfig branch_cfg = {
        .w = rtsp->w, .h = rtsp->h, .fps = rtsp->fps, .bitrate = rtsp->bitrate};
...
lazy_rtsp_factory_create(UnifiedEngine *engine, CapturePipeline *cap,
                         ...
                         const char *tier, BranchConfig *cfg,
    ...
    g_info("[RTSP] Creating LazyRTSPFactory for '%s'  "
           ...
           mount_path, cfg->codec, cfg->w, cfg->h, cfg->fps,
           cairo_enabled ? "TRUE" : "FALSE", cfg->ntp_overlay ? "TRUE" : "FALSE");
    ...
    g_strlcpy(rtsp->codec,      cfg->codec, sizeof(rtsp->codec));
    rtsp->w             = cfg->w;
    rtsp->h             = cfg->h;
    rtsp->fps           = cfg->fps;
    rtsp->bitrate       = cfg->bitrate;
    rtsp->cairo_enabled = cairo_enabled;
    rtsp->ntp_overlay   = cfg->ntp_overlay;
    ...
    char launch[512];
    if (g_strcmp0(tier, "h264") == 0 || g_strcmp0(cfg->codec, "h264") == 0) {
    ...
    } else if (g_strcmp0(tier, "h265") == 0 || g_strcmp0(cfg->codec, "h265") == 0) {
        snprintf(launch, sizeof(launch),
                 "( appsrc name=pay_src is-live=true format=time block=true "
