 ...
 *   [FIX-4] Subscriber callbacks invoked outside stream->lock to prevent
 * deadlock. [FIX-5] stream_manager_force_teardown now silences appsink
 * callbacks before freeing the ManagedStream, preventing a use-after-free crash
 * when the streaming thread fires _on_new_sample mid-teardown.
 *
 * NAMING IMPROVEMENTS:
 ...
 *   [OPT-COPY] _on_new_sample: replaced gst_buffer_ref +
 * gst_buffer_make_writable with gst_buffer_copy_region using
 * FLAGS|TIMESTAMPS|META flags. [OPT-SNAP] Pre-allocated snapshot array
 * eliminates per-frame malloc. [OPT-TEE]  build_encoder_elements receives
 * src_w/src_h from the capture pipeline so it can skip the per-branch G2D when
 * branch resolution equals the tee output resolution. [OPT-PATH]
 * cairo_enabled=FALSE routes the branch to the NV12 tee.
 */
...
static ManagedStream *_managed_stream_new(StreamManager *mgr, const char *lens,
                                          const char *tier);
...
gboolean stream_manager_acquire(StreamManager *mgr,
                                CapturePipeline *capture_pipe, const char *lens,
                                const char *tier, BranchConfig *branch_cfg,
                                gboolean cairo_enabled) {
  g_info("[SMGR] stream_manager_acquire: %s:%s  cairo=%s", lens, tier,
  ...
  if (stream) {
    stream->ref_count++;
    g_info("[SMGR] Reusing existing stream for '%s' (refs=%d)", branch_key,
           stream->ref_count);
  ...
  stream->ref_count = 1;
  stream->capture_pipe = capture_pipe;
  ...
  GstElement **enc_elements = build_encoder_elements(
      stream->branch_name, branch_cfg->codec, branch_cfg->fps,
      branch_cfg->width, branch_cfg->height, capture_pipe->tee_width,
      capture_pipe->tee_height, branch_cfg->bitrate, branch_cfg->bitrate_mode,
      cairo_enabled, branch_cfg->ntp_overlay, is_third, &enc_count);

  if (!enc_elements || enc_count == 0) {
    g_warning("[SMGR] build_encoder_elements returned NULL for '%s'",
              branch_key);
  ...
  g_object_set(stream->appsink, "emit-signals", TRUE, "drop", TRUE,
               "max-buffers", 2, "sync", FALSE, NULL);

  GstAppSinkCallbacks callbacks = {0};
  callbacks.new_sample =
      (GstFlowReturn (*)(GstAppSink *, gpointer))_on_new_sample;
  gst_app_sink_set_callbacks(GST_APP_SINK(stream->appsink), &callbacks, stream,
                             NULL);
  ...
  gboolean use_nv12_tee = !(cairo_enabled || branch_cfg->ntp_overlay);
  stream->branch =
      capture_pipeline_add_branch(capture_pipe, stream->branch_name,
                                  all_elements, total_elements, use_nv12_tee);
  g_free(all_elements);

  if (!stream->branch) {
    g_warning("[SMGR] FAILED to add branch '%s' to capture pipeline",
              stream->branch_name);
...
void stream_manager_release(StreamManager *mgr, const char *lens,
                            const char *tier) {
  g_mutex_lock(&(mgr->mgr_lock));
  char *branch_key = g_strdup_printf("%s:%s", lens, tier);
  ...
  if (found_stream) {
    ...
    if (found_stream->ref_count <= 0) {
       ...
       * This guarantees _on_new_sample won't touch found_stream after we free
       * it. */
      ...
      g_strlcpy(branch_name_copy, found_stream->branch_name,
                sizeof(branch_name_copy));
      CapturePipeline *cap_ref = found_stream->capture_pipe;
       ...
       * remove_branch acquires capture_pipe->branch_mutex. If we hold mgr_lock
       * here and the streaming thread is in _on_new_sample (holding
       * stream->lock and waiting for mgr_lock), we have an AB/BA deadlock.
    ...
    /* Downgraded from g_warning to g_debug: this is expected when
     * force_teardown already cleaned up the stream (e.g., HLS reaper
     * racing with a codec rebuild). */
    g_debug("[SMGR] release: stream '%s' already gone (likely force-torn-down)", branch_key);
  }
...
void stream_manager_force_teardown(StreamManager *mgr, const char *lens,
  ...
  if (doomed_stream) {
    ...
    g_debug("[SMGR] force_teardown: '%s' not active, nothing to do",
            lookup_key);
  }
...
gulong stream_manager_subscribe(StreamManager *mgr, const char *lens,
                                const char *tier, StreamBufferFunc func,
                                gpointer user_data) {
  g_mutex_lock(&(mgr->mgr_lock));
  char *branch_key = g_strdup_printf("%s:%s", lens, tier);
  ...
  g_hash_table_insert(target_stream->subscribers, GUINT_TO_POINTER(handler_id),
                      subscriber);

  g_debug("[SMGR] subscribe '%s:%s'  handler_id=%lu  subscribers=%u", lens,
          tier, handler_id, g_hash_table_size(target_stream->subscribers));
  g_mutex_unlock(&target_stream->lock);
  g_mutex_unlock(&mgr->mgr_lock);
  return handler_id;
}

void stream_manager_unsubscribe(StreamManager *mgr, const char *lens,
                                const char *tier, gulong handler_id) {
  g_mutex_lock(&(mgr->mgr_lock));
  char *branch_key = g_strdup_printf("%s:%s", lens, tier);
  ManagedStream *target_stream = g_hash_table_lookup(mgr->streams, branch_key);
  g_free(branch_key);
  ...
  if (target_stream) {
    ...
    g_debug(
        "[SMGR] unsubscribe '%s:%s'  handler_id=%lu  removed=%s  remaining=%u",
        lens, tier, handler_id, removed ? "YES" : "NO",
     ...
     * force_teardown already cleaned up the stream (e.g., HLS reaper
     * racing with a codec rebuild). */
    g_debug("[SMGR] unsubscribe: stream '%s:%s' not found (likely already torn down)", lens, tier);
  }
  g_mutex_unlock(&mgr->mgr_lock);
}
...
/*
 * [FIX-4]  Subscriber callbacks invoked outside stream->lock (deadlock
 * prevention). [OPT-COPY] Zero-copy buffer delivery via gst_buffer_copy_region
 * header copy. [OPT-SNAP] Pre-allocated snapshot array eliminates per-frame
 * malloc.
 */
static GstFlowReturn _on_new_sample(GstAppSink *sink, gpointer user_data) {
  ...
  if (_perf_debug_enabled()) {
    ...
    if (total % 300 == 0) {
      g_debug(
          "[SMGR-BUF] stream '%s:%s'  delivered=%d  dropped=%d  subscribers=%u",
  ...
  for (guint cb_idx = 0; cb_idx < snap_idx; cb_idx++) {
    ...
    if (header_copy) {
      ...
      /* subscriber->func (engine.g. _on_encoded_buffer in sys_rtsp.c) calls
...
static ManagedStream *_managed_stream_new(StreamManager *mgr, const char *lens,
  ...
  g_debug("[SMGR] ManagedStream created: '%s:%s'  branch='%s'", lens, tier,
          new_stream->branch_name);
  return new_stream;
}
...
static void _managed_stream_free(ManagedStream *dead_stream) {
  if (!dead_stream)
    return;
  g_info("[SMGR] Freeing ManagedStream '%s:%s'  delivered=%d  dropped=%d  "
         "subscribers=%u",
