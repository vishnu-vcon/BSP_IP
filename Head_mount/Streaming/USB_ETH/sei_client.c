    ...
    if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
        ...
        for (gsize i = 0; i < size - 6; i++) {
            ...
            if (is_h264_sei || is_hevc_sei) {
                ...
                if (g_verbose) {
                    long latency_ns = (long)(meta.imu_timestamp_ns - meta.frame_pts_ns);
                    g_print("\n── SEI Frame #%u ────────────────────────────\n", meta.frame_id);
                    g_print("  V4L2 Time : %lu ns (kernel interrupt)\n", (unsigned long)meta.frame_pts_ns);
                    g_print("  IMU Time  : %lu ns (software read)\n", (unsigned long)meta.imu_timestamp_ns);
                    g_print("  Sync Latency: %ld ns (%ld us)\n", latency_ns, latency_ns / 1000);
                    ...
                    long latency_ns = (long)(meta.imu_timestamp_ns - meta.frame_pts_ns);
                    g_print("\rSEI #%u | A(%d,%d,%d)mg G(%d,%d,%d)mdps M(%d,%d,%d)uT | Latency: %ld us   ",
                            meta.frame_id,
                            meta.accel_x_mg, meta.accel_y_mg, meta.accel_z_mg,
                            meta.gyro_x_mdps, meta.gyro_y_mdps, meta.gyro_z_mdps,
                            meta.mag_x_ut, meta.mag_y_ut, meta.mag_z_ut,
