...
static GstPadProbeReturn camera_src_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
    ...
    if (src) {
        base_time = gst_element_get_base_time(src);
        gst_object_unref(src);
    }

    /* PTS is relative to base_time. By adding them, we recover the absolute
     * CLOCK_MONOTONIC timestamp generated by the V4L2 kernel driver during capture! */
                                              ...
                                              gpointer user_data)
{
    ...
    /* CRITICAL: Copy timestamps (PTS/DTS) from the original buffer so downstream
     * elements like mp4mux do not fail with "Buffer has no PTS". */
    gst_buffer_copy_into(sei_buf, buffer, GST_BUFFER_COPY_TIMESTAMPS | GST_BUFFER_COPY_FLAGS, 0, -1);
