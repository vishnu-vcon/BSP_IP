...
static GstPadProbeReturn camera_src_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
    ...
    /* Get the pipeline's base time (time when it started playing) */
    GstElement *src_elem = gst_pad_get_parent_element(pad);
    GstClockTime base_time = 0;
    if (src_elem) {
        base_time = gst_element_get_base_time(src_elem);
        gst_object_unref(src_elem);
    }

    /* PTS is relative to base_time. By adding them, we recover the absolute
     * CLOCK_MONOTONIC timestamp from the V4L2 kernel driver capture interrupt. */
    GstClockTime abs_kernel_time = (pts != GST_CLOCK_TIME_NONE) ? (pts + base_time) : 0;

    SeiMetadata meta;
    memset(&meta, 0, sizeof(meta));
    meta.magic   = SEI_METADATA_MAGIC;
    meta.version = SEI_METADATA_VERSION;
    meta.frame_pts_ns = abs_kernel_time;
                                              ...
                                              gpointer user_data)
{
     ...
     *
     * CRITICAL: We use gst_buffer_copy_deep() instead of gst_buffer_ref()
     * to immediately release the hardware encoder's DMABUF buffer back to
     * its pool. Without this, downstream elements (rtpbin retransmission
     * queue, mp4mux interleave buffer) hold hardware buffer references,
     * starving the encoder pool and freezing the pipeline at ~53 frames.
     *
     * The encoded H.264 frame is only a few KB, so the deep copy into
     * system RAM is negligible compared to the frame interval (33ms).
     *
     * We also copy PTS/DTS/flags from the original buffer onto the SEI
     * buffer so the merged result carries valid timestamps for mp4mux.
     */
    GstBuffer *sei_buf = gst_buffer_new_wrapped(pkt, total);
    gst_buffer_copy_into(sei_buf, buffer,
                         GST_BUFFER_COPY_TIMESTAMPS | GST_BUFFER_COPY_FLAGS, 0, -1);
    GstBuffer *new_buffer = gst_buffer_append(sei_buf, gst_buffer_copy_deep(buffer));
